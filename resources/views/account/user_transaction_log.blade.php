                            ...
                            <div class="col-sm-12 col-xs-12" style="margin-top: 25px;">
                                <form action="" class="searchs">
                                    ...
                                    <div class="form-group col-sm-3">
                                        <label>账户</label>
                                        <div class="col-md-12-select">
                                            <select class="selectpicker bs-select-hidden" data-style="btn-info btn-outline" id="user_account_id" name="user_account_id">
                                    ...
                                    <div class="form-group col-sm-3">
                                        <label>用户类型</label>
                                        <div class="col-md-12-select">
                                            <select class="selectpicker bs-select-hidden" data-style="btn-info btn-outline" id="role" name="role">
                                    ...
                                    <div class="form-group col-sm-3">
                                        <label>用户</label>
                                        <div class="col-md-12-select">
                                            <select class="selectpicker bs-select-hidden" data-style="btn-info btn-outline" id="user_id" name="user_id">
                                                <option value="">请选择用户</option>
                                                @foreach ($users as $user)
                                                <option value="{{Hashid::encode($user->id)}}">{{$user->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                ...
                <div class="panel">
                    <div class="table-responsive">
                        <table class="table table-hover manage-u-table">
                            ...
                            <tr>
                                <td>{{$transactionLog->userAccount->user->name}}</td>
                                <td>{{$transactionLog->userAccount->user->getUserType()}}</td>
                                <td>{{isset($actionMaps[$transactionLog->action]) ? $actionMaps[$transactionLog->action] : $transactionLog->action}}</td>
                                <td>{{isset($transactionLog->paymentMethod)?$transactionLog->paymentMethod->name : ''}}</td>
                                <td>{{ isset($paymentMethodMaps[$transactionLog['payment_method']]) ? $paymentMethodMaps[$transactionLog['payment_method']] : '付款方式不存在' }}</td>
                                <td>{{$transactionLog->userAccount->getPaymentMethod->currency}} {{$transactionLog['amount']}}</td>
