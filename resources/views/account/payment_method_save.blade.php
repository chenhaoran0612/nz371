    ...
    <div class="container-fluid">
        ...
        <div class="row bg-title">
            <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
                <h4 class="page-title">设置{{$paymentmethod->paymentMethodValue}}</h4>
            </div>
        </div>
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-9">
                                                    <select name="method" class="form-control">
                                                        ...
                                                        @foreach ($paymentMethodMap as $key => $map)
                                                        <option value="{{$key}}" {{$paymentmethod->method == $key ? 'selected':''}}>{{$map}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <div class="insert-table">
                                            @foreach ($paymentMethodConfig as $key => $config)
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="control-label col-md-3">{{$config}}</label>
                                                        <div class="col-md-9">
                                                            <input type="text" class="form-control parame" data-name="{{$key}}" value="{{isset($paymentmethod->config[$key])? $paymentmethod->config[$key] : ''}}">
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-9">
                                                    <select class="selectpicker bs-select-hidden" data-style="btn-info btn-outline" disabled>
                                                        @foreach ($currencyMap as $currency)
                                                            <option value="{{$currency}}" {{$paymentmethod->currency == $currency?'selected' : ''}}>{{$currency}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                        ...
                                        <input type="hidden"  value="{{Hashid::encode($paymentmethod->id)}}" name="id">
                                    </div>
...
@section('extend_js')
    ...
    <script type="text/javascript">
        ...
        $('.paymentmethod_submit').click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "/account/paymentmethod/save",
                dataType: "json",
                data : data,
                success: function(d) {
                    if(d.result){
